...
const initializationParams = require("./data").initializationParamsV2;
...
function successSignature() {
    server.respondWith("POST", initializationParams.authenticationEndpoint,
        [
            200,
...
function successUploadResponse() {
    server.respondWith("POST", "https://upload.imagekit.io/api/v2/files/upload",
        [
            200,
            { "Content-Type": "application/json" },
...
describe("File upload", function () {
    ...
    it('Auth endpoint non 200 status code handling', async function () {
        ...
        await sleep();
        sinon.assert.calledWith(callback, { message: "Invalid response from authenticationEndpoint. The SDK expects a JSON response with one field token.", help: "" }, null);
    });
    ...
    it('Boolean handling', async function () {
        ...
        const arg = server.requests[0].requestBody;
    ...
    it('Overriding public key and authentication endpoint', async function () {
        const newAuthEndpoint = "http://test/auth-override";
        const newPublicKey = "override_public_key";
        ...
        expect(server.requests.length).to.be.equal(2);
        server.respondWith("POST", newAuthEndpoint,
            [
                200,
                { "Content-Type": "application/json" },
                JSON.stringify({
    ...
    it('check custom XHR object is used', async function () {
        ...
        const fun = function () { return "hello from function" };
        xhr.onprogress = fun;
        const fileOptions = {
            fileName: "test_file_name",
            file: "test_file",
    ...
    it('$ResponseMetadata assertions using callback', async function () {
        ...
        const callBackArguments = callback.args[0];
        expect(callBackArguments.length).to.be.eq(2);
        const callbackResult = callBackArguments[1];
